...
export default function Sidebar({sidebarOpen, setSidebarOpen, activeTab,setActiveTab}) {
  return (
    <div className={`${sidebarOpen ? "translate-x-0":"-translate-x-full"} fixed inset-y-0 left-0 z-50 w-80 bg-white/10 backdrop-blur-2xl border-r border-white/20 transform transition-all duration-500 ease-out lg:translate-x-0 lg:static lg:inset-0`}>
      <div className="flex items-center justify-between h-19 px-8 border-b border-white/20">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-white/60 font-medium">PREMIUM STRATEGY</p>
          </div>
        </div>
        <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white/70 hover:text-white p-2 rounded-xl hover:bg-white/10 transition-colors">
          <X className="h-6 w-6" />
        </button>
      </div>
      <nav className="mt-8 px-6 space-y-2">
        {menuItems.map((items) => {
          return <button key={items.id} className={`w-full flex items-center px-6 py-4 text-left rounded-2xl transition-all duration-300 group relative overflow-hidden 
          ...
          onClick={() => {setActiveTab(items.id); setSidebarOpen(true)}} >

            {activeTab === items.id && (
<div className={`absolute inset-0 bg-gradient-to-r ${items.grandient} opacity-30 rounded-2xl`}>
          
          </div>
            )}
          
          <div className={`p-2 rounded-xl bg-gradient-to-r mr-4 group-hover:scale-110 transition-all duration-300 relative z-10 ${items.grandient}`}>
